...
class EvalArgs:
    ...
    ckpt_dir: str = ""
    generator: PackedHierarchicalCausalTransformerGeneratorArgs = field(
        default_factory=PackedHierarchicalCausalTransformerGeneratorArgs
    )
...
def launch_eval(cfg: EvalArgs):
    ...
    model, tokenizer, regex_pool, train_cfg = load_consolidated_model_and_tokenizer(
        consolidate_path,
...
def main():
    ...
    class DummyArgs:
        name: str
        model: HierarchicalArgs

    @dataclass
    class HierarchicalArgs:
        dim: int

    Then you can pass model.dim=32 to change values in HierarchicalArgs
